...
float promedioTrabajadorColumnas(int filas,float suma);
    ...
    for (int i = 0; i < filas; i++){
        for (int j = 0; j < columnas; j++){
            if(j==j+1){
                ...
                sumaColumnas = sumaColumnas + matriz[i][j+1];

            }
        }
        printf("\nTotal\tPromedio\n");    
        printf("\n[%i]%.2f\t",i+1,sumaColumnas);
        printf("[%i]%.2f",i+1,(sumaColumna1 = promedioTrabajadorColumnas(filas,sumaColumnas)));
    }
